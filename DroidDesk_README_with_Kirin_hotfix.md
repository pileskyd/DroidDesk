# Huawei / Honor / Kirin / Mali / Maleoon GPU Hotfix

> [!IMPORTANT]
> If you are using a Huawei / Honor device with a Kirin chip, Mali GPU, Maleoon GPU, or Immortalis GPU, do **not** use the default Zink/Turnip GPU path. Turnip is mainly for Qualcomm Adreno, and Zink/Vulkan on Kirin/Mali/Maleoon may freeze Termux-X11 after a short time.
>
> For these devices, use the Kirin/Mali hotfix. It switches the desktop to a safer `virglrenderer-android` / `virpipe` path, disables unstable XFCE compositing, prevents Firefox from killing the X11 session through broken GPU/WebGL paths, and adds a software fallback launcher.

## Huawei / Kirin Installation Path

First run the normal installer:
...
Then apply the Kirin/Mali hotfix:

```bash
chmod +x termux-kirin-x11-hotfix-v2.sh
bash ./termux-kirin-x11-hotfix-v2.sh
```
...
If the desktop still freezes or crashes, try VirGL without ANGLE:

```bash
TERMUX_VIRGL_BACKEND=plain bash ~/start-x11.sh
```

If the GPU path is still unstable on your ROM/GPU driver, use the safe software fallback:

```bash
bash ~/start-x11-safe.sh
```
...
```bash
cat ${TMPDIR:-/data/data/com.termux/files/usr/tmp}/virgl-kirin.log
```

### Why this is needed

Huawei Kirin / Mali / Maleoon devices can expose unstable Vulkan/OpenGL behavior under Termux-X11. A common failure pattern is:

```text
MESA: error: ZINK: failed to choose pdev
libEGL warning: DRI3 error: Could not get DRI3 device
GL_EXT_shader_texture_lod unsupported
Crash Annotation GraphicsCriticalError: Couldn't sanitize GL_RENDERER "virgl (...)"
```

These warnings mean the session is leaking into the wrong GPU path or Firefox/WebGL is hitting an unsupported VirGL/Mesa feature. The hotfix keeps the desktop on the VirGL path and runs Firefox in a safer software-rendered mode.
